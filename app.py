...
def check_and_award_achievements(username: str, bet: dict, payout: int):
    ...
    if not has_badge(username, "gem_hunter"):
        if bet.get("category") == "Hidden Gem":
            # Count correct hidden gem bets
            all_bets = get_bets("closed")
            gem_bets = [b for b in all_bets if b.get("category") == "Hidden Gem"]
            correct  = 0
            for gb in gem_bets:
                e = user_entry(gb["id"], username)
                if e and e["option"] == gb.get("result"):
                    correct += 1
            if correct >= 5:
    ...
    if not has_badge(username, "raid_master"):
        if bet.get("category") == "Raid / Shoutout":
            all_bets  = get_bets("closed")
            raid_bets = [b for b in all_bets if b.get("category") == "Raid / Shoutout"]
            correct   = 0
            for rb in raid_bets:
                e = user_entry(rb["id"], username)
                if e and e["option"] == rb.get("result"):
                    correct += 1
            if correct >= 10:
    ...
    if not has_badge(username, "indie_scout"):
        all_entries = db().table("bet_entries").select("bet_id") \
                          .eq("username", username).execute().data or []
        bet_ids = [e["bet_id"] for e in all_entries]
        if bet_ids:
            placed_bets = db().table("bets").select("vtuber_name") \
                              .in_("id", bet_ids).execute().data or []
            unique_vt = len(set(b["vtuber_name"].lower() for b in placed_bets))
            if unique_vt >= 20:
    ...
    with col:
        ...
        <div style="text-align:center;padding:48px 0 32px;">
            ...
            <div style="font-family:'Syne',sans-serif;font-size:3rem;font-weight:800;
                        color:#e8f0ff;line-height:1.1;margin-bottom:10px;">VTuberBets</div>
        ...
        with tab_login:
            ...
            if st.button("Login", use_container_width=True, key="btn_login"):
                if not username.strip() or not password.strip():
                    set_toast("error", "Please fill in both fields.")
                    st.rerun()
                else:
        ...
        with tab_register:
            ...
            new_pass2 = st.text_input("Confirm password", key="reg_pass2",
                                      type="password", placeholder="Repeat password")
            if st.button("Create Account", use_container_width=True, key="btn_register"):
                un = new_user.strip()
                errs = []
                if len(un) < 2:           errs.append("Username must be at least 2 characters.")
                if len(un) > 24:          errs.append("Username must be 24 characters or fewer.")
                if len(new_pass) < 6:     errs.append("Password must be at least 6 characters.")
                if new_pass != new_pass2: errs.append("Passwords do not match.")
                if errs:
                    set_toast("error", errs[0])
                    st.rerun()
                else:
                    ok, msg = register_user(un, new_pass)
                    if ok:
        ...
        st.markdown("""
        <div style="text-align:center;margin-top:28px;display:flex;
    ...
    with col:
        ...
        <div style="text-align:center;padding:32px 0 24px;">
            <div style="font-family:'Syne',sans-serif;font-size:1.6rem;
                        font-weight:800;color:#e8f0ff;margin-bottom:8px;">
                One last thing
            </div>
            <div style="color:#3a5580;font-size:0.88rem;">
        ...
        role_descs = {
            "Watcher":  "You watch streams, follow indie VTubers, and want to bet on their chaos.",
            "Streamer": "You are a VTuber or streamer yourself.",
            "Clipper":  "You create clips and highlight reels of indie VTubers.",
        }

        for role, desc in role_descs.items():
            css = {"Watcher":"role-watcher","Streamer":"role-streamer","Clipper":"role-clipper"}[role]
            st.markdown(f"""
            <div style="background:#0b0f1e;border:1px solid #1e3060;border-radius:10px;
                        padding:16px 20px;margin-bottom:10px;">
                <span class="profile-role {css}">{role}</span>
                <div style="color:#5577aa;font-size:0.85rem;margin-top:6px;">{desc}</div>
            </div>
            """, unsafe_allow_html=True)
            if st.button(f"I am a {role}", key=f"role_{role}", use_container_width=True):
                set_user_role(st.session_state.username, role)
                set_toast("success", f"Welcome, {role}! Your account is ready.")
                nav("home")
...
def main():
    ...
    if needs_role_selection(st.session_state.username):
        page_role_select()
        return
